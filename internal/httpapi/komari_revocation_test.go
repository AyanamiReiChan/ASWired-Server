...
func TestKomariSessionRevocation(t *testing.T) {
	...
	if _, err := a.DB.SaveMemberRecord(ctx, u, store.Record{Collection: "members", ID: u.ID, Data: map[string]any{"application": "komari"}}, false); err != nil {
		t.Fatal(err)
	}
	...
	a.issue(ticketOut, u)
	u.TokenVersion++
	if err := a.DB.UpdateUser(ctx, u); err != nil { t.Fatal(err) }
	requireStatus(t, bridgeRequest(t, h, a.Config.KomariBridgeSecret, "introspect", map[string]any{"session": session}), 401)
